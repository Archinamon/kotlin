...
open class SerializationPluginDeclarationChecker : DeclarationChecker {
    ...
    private fun checkClassWithCustomSerializer(descriptor: ClassDescriptor, declaration: KtDeclaration, trace: BindingTrace) {
        val annotationPsi =
            descriptor.findSerializableAnnotationDeclaration() ?: declaration.findSerializableAnnotationDeclarationSlowPath()
        checkCustomSerializerMatch(descriptor.module, descriptor.defaultType, descriptor, annotationPsi, trace, declaration)
        checkCustomSerializerIsNotLocal(descriptor.module, annotationPsi, descriptor, trace, declaration)
    }

    private fun PsiElement.findSerializableAnnotationDeclarationSlowPath(): KtAnnotationEntry? {
        val modifierList = children.asSequence().filterIsInstance<KtDeclarationModifierList>().singleOrNull() ?: return null
        return modifierList.children.asSequence().filterIsInstance<KtAnnotationEntry>()
            .filter { it.shortName == serializableAnnotationFqName.shortName() && it.valueArguments.size == 1 && it.valueArguments[0].getArgumentName()?.asName?.asString() == "with" }
            .singleOrNull()
    }
    ...
    private fun analyzePropertiesSerializers(trace: BindingTrace, serializableClass: ClassDescriptor, props: List<SerializableProperty>) {
        ...
        props.forEach {
            ...
            if (serializer != null) {
                ...
                checkCustomSerializerIsNotLocal(it.module, null, it.descriptor, trace, propertyPsi)
    ...
    private fun AbstractSerialGenerator.checkType(
        ...
        if (serializer != null) {
            ...
            checkCustomSerializerIsNotLocal(module, null, type, trace, fallbackElement)
    ...
    private fun checkCustomSerializerIsNotLocal(
        ...
        if (DescriptorUtils.isLocal(serializerDescriptor)) {
            val element = annotationElement ?: declarationElement.findSerializableAnnotationDeclarationSlowPath() ?: declarationElement
