...
class KtValueArgumentTest : KotlinTestWithEnvironment() {
    fun testCorruptedJvmName() {
        ...
        val ktValueArgumentStubs = mutableListOf<KotlinValueArgumentStub<*>>()
        KtFileStubBuilder().buildStubTree(ktFile).cast<PsiFileStub<*>>().forEachDescendantOfType {
            if (it is KotlinValueArgumentStub<*>) {
                ktValueArgumentStubs.add(it)
            }
        }
        assertEquals(4, ktValueArgumentStubs.size)
        assertTrue(ktValueArgumentStubs.all { it.psi.stub != null })
        assertEquals(1, ktValueArgumentStubs.mapNotNull { it.psi.getArgumentExpression() }.size)
    }
